...
'''
파라미터: zipline결과 데이터프레임, backtesting했던 주가데이터 프레임
기능: 볼린저 밴드 값과 거래 포인트가 추가된 데이터프레임을 그래프로 나타냄
'''
def make_graph(result, base_data):    
    ...
    base_data = base_data.loc[result.index[0] - 7 * Hour(): result.index[-1]]
    ...
    ax1.plot(index, result.portfolio_value, color='y')   
    ax1.plot(index[result.buy == True], result.portfolio_value[result.buy == True], '^', label= 'buy') # 메수 지점에 ^표시
    ax1.plot(index[result.sell == True], result.portfolio_value[result.sell == True], 'v', label= 'sell') # 매도 지점에 v표시
...
if __name__ == "__main__":
    start = pd.to_datetime('2013-01-01').tz_localize('US/Eastern')
    end = pd.to_datetime('2019-12-30').tz_localize('US/Eastern')

    base_data = pd.read_csv('./bbands/000660_b5.csv')
    # base_data = pd.read_csv('result.csv')
    ...
    result.to_csv('./bbands/000660_b5_zipline.csv') # zipline결과를 파일로 저장
