...
class Strategy:
    ...
    def make_trade_point(self, df, tech_indicator=None, indi_count=None):
        if tech_indicator: # 어떤 보조지표를 함께 확인할 것인지 정할 수 있다.
            result = self.for_tech_indicator(df, tech_indicator)        
            return result
    ...
    기능: bbcandle과 보조지표의 신호를 확인하여 최종 거래 신호를 발생시킨다.
    리턴: 데이터프레임에 실제 매수매도 시그널을 생성    '''
    def for_tech_indicator(self, df, tech_indicator):
    ...
    def find_stock_by_bb(self, strategy, stockmarket=None, item_list=None, check_date='today', dtype='D'):
        signal = Signal()
        df = signal.check_stock(stockmarket=stockmarket, item_list=item_list, check_date=check_date, dtype=dtype)
        result = self.make_trade_point(df, strategy)
