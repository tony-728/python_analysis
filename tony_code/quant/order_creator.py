...
def BBcandle(df, period=10, nbdevup=2, nbdevdn=2, up_pct=2.0, down_pct=0.5, check_period=2):
...
def RSI(df, timeperiod=14):  #, up_pct=70, donw_pct=30
...
def STOCH(df, fastk_period=5, slowk_period=3, slowd_period=3, up_pct=80, down_pct=20):
...
def make_tech(file_contents, df, tech_idx, order_idx):
    ...
    for idx, tech in enumerate(techs):
        if idx == 0:
            ...
            snd_df = eval(tech)
            fir_df = merge_all_df(fir_df, snd_df)

    fir_df = merge_all_df(fir_df, df)
...
def make_strategy(file_contents, order_idx):
    f = copy.deepcopy(file_contents)
    order_list = []
...
def make_trading(order_list, trade_df, code):
    trade_list=[]
    ...
    for order in order_list:
        for _, trade in trades.iterrows():            
            if eval(order[0]): # if trade.rsi > 70 & trade.bbcandle_buy:
                order_data = eval(order[1].replace(' ','').replace('(', '(trade,')) # buy(trade, stock)
...
if __name__ == "__main__":
    ...
    code, startdate, enddate, dtype, f, tech_idx, order_idx = split_order(f)
    ...
    df = get_stock(code=code, startdate=startdate, enddate=enddate, dtype=dtype)
    # print(df)

    trade_df = make_tech(f, df, tech_idx, order_idx)
    # print(trade_df)
    # trade_df.to_csv('check_order.csv')

    order_list = make_strategy(f, order_idx)
    # print(order_list)

    order_df = make_trading(order_list=order_list, trade_df=trade_df, code=code)
    # print(order_df)
    order_df.to_json('order.json', orient='records', force_ascii=False)
