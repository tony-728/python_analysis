...
def check_STOCH_CASE1(df, up_pct=80, down_pct=20):
    ...
    result_case1_df = merge_all_df(stoch_buy1_df, stoch_sell1_df)
...
def check_STOCH_CASE2(df, up_pct=80, down_pct=20):
    ...
    stoch_buy2_df = pd.DataFrame(stoch_buy2.items(), columns=['Date', 'stoch2_buy'])
    ...
    stoch_sell2_df = pd.DataFrame(stoch_sell2.items(), columns=['Date', 'stoch2_sell'])
    stoch_sell2_df.set_index('Date', inplace=True)

    result_case2_df = merge_all_df(stoch_buy2_df, stoch_sell2_df)
...
def check_stock(stockmarket=None, item_list=None, check_date='today', dtype='D'):
    ...
    def make_signal(df):
        ...
        return_df = merge_all_df(bbcandle, rsi, macd, stoch)
    ...
    if dtype == 'D':
        for no, stock in enumerate(stocklist):
            df = get_stock(code=stock, startdate=check_date, enddate=check_date)
        ...
        for no, stock in enumerate(stocklist):
            df = get_stock(code=stock, startdate=check_date, enddate=check_date, dtype='W')
...
if __name__ == "__main__":
    df = get_stock('000660', '2015-01-01')
    # print(df)
    stoch = get_STOCH(df)
    # print(stoch)
