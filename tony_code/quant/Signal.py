...
class Signal:
    ...
    '''
    def check_bbcandle(self, df, period=2): 
    ...
    def check_RSI(self, df, up_pct=70, donw_pct=30):
        rsi_buy = df.rsi <= 30
    ...
    리턴: 각각의 주식에대한 시그널    '''
    def check_stock(self, stockmarket=None, item_list=None, check_date='today', dtype='D'):
        gathering = Gathering()
        tech_indi = TechIndi()
        ...
        if dtype == 'D':
            for no, stock in enumerate(stocklist):
                ...
                bband = tech_indi.get_BBand(df=df)
                rsi = tech_indi.get_RSI(df=df)
                macd = tech_indi.get_MACD(df=df)
                stoch = tech_indi.get_STOCH(df=df)  

                # 기본전략 시그널
                bbcandle = self.check_bbcandle(df=bband)
                # 보조지표 시그널
                rsi = self.check_RSI(df=rsi)
                macd = self.check_MACD(df=macd)    
                stoch = self.check_STOCH(df=stoch)
            

                df_for_trade = gathering.merge_all_df(bbcandle, rsi, macd, stoch)
